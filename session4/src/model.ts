...
export const createModel = (persons: Person[], employees: Employee[], filter: (d:Data) => boolean = () => true): Model => {
    const employeeMap: { [eId: number]: Employee } = {}
    const employeeData = (p: Person) => p.employeeId === undefined ? undefined : employeeMap[p.employeeId]
    employees.forEach(e => employeeMap[e.employeeId] = e)

    const personData: () => Data[] = () => persons
        .map(p => ({ ...p, ...employeeData(p)}))
        .filter(filter)
